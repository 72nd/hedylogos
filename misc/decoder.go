...
func (d *docDecoder) canSkipDefault(t xml.Token, name xml.Name) (bool, error) {
	if name.Local != "key" {
		return false, nil
	}
	switch tt := t.(type) {
	case xml.StartElement:
		if tt.Name.Local != "default" {
			return false, nil
		}
		for {
			nxt, err := d.token()
...
func (d *docDecoder) expectEnd(tok xml.Name) error {
	for {
		...
		canSkip, err := d.canSkipDefault(t, tok)
		if err != nil {
			return nil
		}
		if canSkip {
		...
		return fmt.Errorf("unexpected token in expectEnd: %T: %#v", t, t)
	}
}
